...
class MyTagsSearchFAIL extends Simulation {
	...
	val scn = scenario("MyTagsSearchFAIL")
		.exec(http("request_0")
			...
			.headers(headers_0)
			.body(RawFileBody("/mytagssearchfail/0000_request.json")))
		.pause(1)
		.exec(http("request_1")
			.get(uri2 + "/?code=650cb76b0352205f6d6081e3a345004f&state=2f11f8654b934454a945726fba582c99")
			.headers(headers_1))
		.pause(6)
		.exec(http("request_2")
			.post("/geolocation/v1/geolocate?key=AIzaSyB2h2OuRcUgy5N-5hsZqiPW6sH3n_rptiQ")
			.headers(headers_0)
			.body(RawFileBody("/mytagssearchfail/0002_request.json")))
		.pause(4)
		.exec(http("request_3")
		...
		.exec(http("request_4")
			...
			.post(uri3 + "/getById")
			.headers(headers_9)
			.body(RawFileBody("/mytagssearchfail/0009_request.json"))))
		.pause(3)
		.exec(http("request_10")
			.post(uri3 + "/location/near")
			.headers(headers_5)
			.body(RawFileBody("/mytagssearchfail/0010_request.json")))
