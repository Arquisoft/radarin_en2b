...
class MyTags extends Simulation {
	...
	val scn = scenario("MyTags")
		.exec(http("request_0")
			.get("/?code=df4e859761cbbe79cf1faebcf215c748&state=2a666898d0b3485d99afdb9ecb938974")
			.headers(headers_0))
		.pause(4)
		.exec(http("request_1")
			.post(uri1 + "?key=AIzaSyB2h2OuRcUgy5N-5hsZqiPW6sH3n_rptiQ")
			.headers(headers_1)
			.body(RawFileBody("/mytags/0001_request.json")))
		.pause(2)
		.exec(http("request_2")
		...
		.exec(http("request_3")
			...
			.post(uri3 + "/add")
			.headers(headers_4)
			.body(RawFileBody("/mytags/0004_request.json")),
            http("request_5")
			...
			.post(uri3 + "/getById")
			.headers(headers_7)
			.body(RawFileBody("/mytags/0007_request.json")),
            http("request_8")
			.post(uri3 + "/location/near")
			.headers(headers_8)
			.body(RawFileBody("/mytags/0008_request.json"))))
